...
const Tooltip = ({ content, children, position = 'top', className = '', icon = false }: TooltipProps) => {
  ...
  if (icon) {
    return (
      <div className={`relative inline-block ${className}`}>
        ...
        {isVisible && (
          <div
            className={`absolute z-50 px-3 py-2 text-xs text-white bg-gray-800 rounded-lg shadow-lg whitespace-nowrap ${positionClasses[position]} transition-opacity duration-200`}
            role="tooltip"
          >
            {content}
